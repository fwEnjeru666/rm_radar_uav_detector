...
private:
    ...
    void publishResults(const pcl::PointCloud<pcl::PointXYZ>::Ptr& detected_cluster,
                        ...
                        Eigen::Vector4f& detected_centroid,
                        double confidence,
    ...
    void publishClusterDebugMarkers(const std::vector<ClusterDebugInfo>& debug_infos,
                                    const std::string& frame_id,
                                    const ros::Time& stamp);
    void publishTrackState(const ros::Time& stamp, const BBox3D* bbox_hint);
    bool updateTrackAabbPoints(const BBox3D& bbox);

    bool extractBestClusterInfo(const std::vector<ClusterDebugInfo>& debug_infos,
    ...
    std::array<geometry_msgs::Point, 8> last_track_aabb_points_{};
    bool has_track_aabb_points_{false};
};
